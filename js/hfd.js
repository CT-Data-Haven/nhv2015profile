...
var chartAttr = {
    fullwidth: 543,
    fullheight: 60,
    top: 0, right: 40, bottom: 20, left: 160,
    zoomFull: 360,
    r: 16
};
...
d3.queue()
    .defer(d3.json, 'json/hfd_shape.json')
    .defer(d3.csv, 'data/hfd_long2015.csv')
    .await(init);
...
function drawMap(topo) {
    ...
    var polygons = svg.append('g')
        ...
        .selectAll('path')
        .data(topojson.feature(topo, topo.objects.hfd_shape).features)
        .enter().append('path')
            .attr('d', path)
            .attr('class', 'polygon');

    // use topojson mesh to make path of each town, add a class
    // not sure why class isn't working--something with not having selectAll
    svg.append('path')
        .attr('transform', 'translate(' + mapAttr.left + ',' + mapAttr.top + ')')
        .datum(topojson.mesh(topo, topo.objects.hfd_shape, function(a, b) {
            return a.properties.Town !== b.properties.Town;
        }))
        .attr('class', 'town')
        .attr('stroke', '#4e4e4e')
    ...
    svg.append('path')
        .attr('transform', 'translate(' + mapAttr.left + ',' + mapAttr.top + ')')
        .datum(topojson.merge(topo, topo.objects.hfd_shape.geometries))
        .attr('class', 'town')
        .attr('stroke', '#4e4e4e')
        .attr('stroke-width', 1.5)
        .attr('fill', 'none')
...
function makeTable(table) {
    ...
    // add Neighborhood to beginning of indicators
    indicators.unshift('Town');
